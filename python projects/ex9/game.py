...
if __name__ == "__main__":
	...
	# with the board's cars data into the alias "cars_to_board"
	for name, val in cars_to_board.items():
		...
		# and adds it to the board (type Board) if its possible according to
		# the board data.
		if 2 <= car_length <= 4 and (name in LEGAL_CAR_NAMES) and \
			...
			_in_board = True  # bool represent whether all the car
			# coordinates in the board's boundaries. True until proved
			# otherwise.
			for cor in _car.car_coordinates():
				if cor not in board.cell_list():
					_in_board = False
					break
			if _in_board:
				board.add_car(_car)
