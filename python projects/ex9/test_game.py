...
class Helper:
    ...
    def finishes_with_exact_moves(self, car_cfg, moves):
        ...
        not_enough_moves_st = "\n".join(moves[:-1])
        print(not_enough_moves_st)
        print(self._run_game_process(car_cfg, not_enough_moves_st) == '')
    ...
    def fails_with_given_moves(self, car_cfg, moves):
        moves_st = "\n".join(moves)
        print(moves_st)
        print(self._run_game_process(car_cfg, moves_st))
        err = self._run_game_process(car_cfg, moves_st)
...
def test_valid_simple():
    ...
    test_helper.finishes_with_exact_moves(cfg_file, ["O,u"] + ["R,r"] * 5 +
                                                    ["R,l", "R,r", "R,r"])
